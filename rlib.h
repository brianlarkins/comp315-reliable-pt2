...
/* Network failure injection (for testing student submissions).
 * Each is an integer percentage 0..100 applied independently per packet
 * in each direction. opt_netfx_seed is logged at startup so failing runs
 * can be replayed. */
extern int opt_netfx_loss;
extern int opt_netfx_corrupt;
extern int opt_netfx_dup;
extern unsigned int opt_netfx_seed;
