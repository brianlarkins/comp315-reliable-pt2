...
int                          opt_netfx_loss;
int                          opt_netfx_corrupt;
int                          opt_netfx_dup;
unsigned int                 opt_netfx_seed;
...
static void usage (void) {
  fprintf (stderr,
      "usage: %s [-d] [-l] [-t timeout] [-w window]\n"
      "          [-L loss%%] [-C corrupt%%] [-P dup%%] [-S seed]\n"
      "          udp-port [host:]udp-port\n",
      progname);
  exit (1);
}
...
int main (int argc, char **argv) {
  ...
  while ((opt = getopt_long (argc, argv, "dlt:w:L:C:P:S:", o, NULL)) != -1)
    switch (opt) {
      case 'd':
        opt_debug = 1;
        break;
      ...
      case 't':
        ...
        break;
      case 'L':
        opt_netfx_loss = atoi (optarg);
        break;
      case 'C':
        opt_netfx_corrupt = atoi (optarg);
        break;
      case 'P':
        opt_netfx_dup = atoi (optarg);
        break;
      case 'S':
        opt_netfx_seed = (unsigned int) strtoul (optarg, NULL, 0);
        break;
      default:
        usage ();
        break;
  ...
  if (optind + 2 != argc || c.window < 1 || c.timeout < 10)
    usage ();
  if (opt_netfx_loss < 0 || opt_netfx_loss > 100 ||
      opt_netfx_corrupt < 0 || opt_netfx_corrupt > 100 ||
      opt_netfx_dup < 0 || opt_netfx_dup > 100) {
    fprintf (stderr, "%s: -L/-C/-P must be in 0..100\n", progname);
    exit (1);
  }
  if (opt_netfx_loss || opt_netfx_corrupt || opt_netfx_dup) {
    if (!opt_netfx_seed)
      opt_netfx_seed = (unsigned int) (time (NULL) ^ getpid ());
    srandom (opt_netfx_seed);
    fprintf (stderr, "[netfx] seed=%u loss=%d%% corrupt=%d%% dup=%d%%\n",
             opt_netfx_seed, opt_netfx_loss, opt_netfx_corrupt, opt_netfx_dup);
  }
  c.timer = c.timeout / 5;
  local = argv[optind];
